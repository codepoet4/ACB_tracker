  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        ...
        {needsPrice && <div><label style={S.label}>Price/Share</label><input type="text" inputMode="decimal" value={tx.pricePerShare} onChange={e => { const v = e.target.value; if (v === "" || /^\d*\.?\d{0,6}$/.test(v)) onChange({ ...tx, pricePerShare: v }); }} onBlur={e => { const n = parseFloat(e.target.value); const u = { ...tx }; if (!isNaN(n)) u.pricePerShare = n.toFixed(6).replace(/0{1,4}$/, ""); const sh = Number(u.shares), pr = Number(u.pricePerShare), comm = Number(u.commission) || 0; if (sh > 0 && pr > 0 && !(Number(u.amount) > 0)) u.amount = (sh * pr + comm).toFixed(6).replace(/0{1,4}$/, ""); onChange(u); }} style={S.input} placeholder="0.00" /></div>}
        {needsAmount && <div><label style={S.label}>Amount ($)</label><input type="text" inputMode="decimal" value={tx.amount} onChange={e => { const v = e.target.value; if (v === "" || /^\d*\.?\d{0,6}$/.test(v)) onChange({ ...tx, amount: v }); }} onBlur={e => { const n = parseFloat(e.target.value); const u = { ...tx }; if (!isNaN(n)) u.amount = n.toFixed(6).replace(/0{1,4}$/, ""); const sh = Number(u.shares), amt = Number(u.amount), comm = Number(u.commission) || 0; if (sh > 0 && amt > 0 && !(Number(u.pricePerShare) > 0)) u.pricePerShare = ((amt - comm) / sh).toFixed(6).replace(/0{1,4}$/, ""); onChange(u); }} style={S.input} placeholder="0.00" /></div>}
