...
function exportSchedule3PDF(report, portfolioName, year) {
  ...
  autoTable(doc, {
    ...
    foot: [[
      ...
      { content: "", styles: { halign: "center" } },
      { content: f(report.totalProceeds), styles: { halign: "right" } },
      { content: f(report.totalACB), styles: { halign: "right" } },
      { content: f(report.totalOutlays), styles: { halign: "right" } },
      { content: f(report.net), styles: { halign: "right" } },
    ]],
...
const S = {
  ...
  nav: { display: "flex", gap: 4, marginTop: 12 },
  navBtn: (active) => ({ flex: 1, padding: "8px 4px", borderRadius: "8px 8px 0 0", fontSize: 12, fontWeight: 500, border: "none", cursor: "pointer", textAlign: "center", background: active ? "#0f1219" : "#252d3d", color: active ? "#fff" : "#9ca3af" }),
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
        ...
        {needsPrice && <div><label style={S.label}>Price/Share</label><input type="text" inputMode="decimal" value={tx.pricePerShare} onChange={e => { const v = e.target.value; if (v === "" || /^\d*\.?\d{0,6}$/.test(v)) onChange({ ...tx, pricePerShare: v }); }} onBlur={e => { const n = parseFloat(e.target.value); const u = { ...tx }; if (!isNaN(n)) u.pricePerShare = n.toFixed(6).replace(/0{1,4}$/, ""); const sh = Number(u.shares), pr = Number(u.pricePerShare); if (sh > 0 && pr > 0 && !(Number(u.amount) > 0)) u.amount = (sh * pr).toFixed(6).replace(/0{1,4}$/, ""); onChange(u); }} style={S.input} placeholder="0.00" /></div>}
        {needsAmount && <div><label style={S.label}>Amount ($)</label><input type="text" inputMode="decimal" value={tx.amount} onChange={e => { const v = e.target.value; if (v === "" || /^\d*\.?\d{0,6}$/.test(v)) onChange({ ...tx, amount: v }); }} onBlur={e => { const n = parseFloat(e.target.value); const u = { ...tx }; if (!isNaN(n)) u.amount = n.toFixed(6).replace(/0{1,4}$/, ""); const sh = Number(u.shares), amt = Number(u.amount); if (sh > 0 && amt > 0 && !(Number(u.pricePerShare) > 0)) u.pricePerShare = (amt / sh).toFixed(6).replace(/0{1,4}$/, ""); onChange(u); }} style={S.input} placeholder="0.00" /></div>}
